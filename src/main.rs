...
fn dedup_from_set (filepath : &Path, checksums : &HashSet<String>) -> bool {
    match checksum(&filepath) {
        Ok(chksum) => {
            let found = match checksums.contains(&chksum) {
                true  => { let _ = remove_file(filepath); true },
                false => false ,
            };
            found
        },
        Err(e) => {
            eprintln!("Skipping {} - {}", &filepath.to_str().unwrap(), e.to_string());
            false 
        }
    }
}
...
fn list_file_to_set (filepath : &Path) -> Result<HashSet<String>> {
    ...
    let remote : Box<std::io::Read> = match filepath.to_str().unwrap() {
...
fn main () {
    ...
    for file in WalkBuilder::new(&local).hidden(false).follow_links(false).build().filter_map(|x| x.ok()) {
        if file.path().is_dir() == true {continue;}
        total_count += 1; 
